...
import (
	...
	fcfsdriver "vazmin.github.io/fastcfs-csi/pkg/fcfs-driver"
...
func NewTestConfigMap(c clientset.Interface, ns *v1.Namespace, cm *v1.ConfigMap) *TestConfigMap {
...
func NewTestSecret(c clientset.Interface, ns *v1.Namespace, sec *v1.Secret) *TestSecret {
	return &TestSecret{
		client: c,
		ns:     ns,
		secret: sec,
	...
	if t.storageClass != nil {
		...
		if len(t.storageClass.AllowedTopologies) > 0 {
			...
			for _, v := range t.persistentVolume.Spec.NodeAffinity.Required.NodeSelectorTerms[0].MatchExpressions {
				if v.Key == driver.TestOptions.TopologyKey {
					keyFound = true
...
//func (t *TestPersistentVolumeClaim) UpdateStorageClass(scName *string) {
//	t.persistentVolume.Spec.StorageClassName = *scName
//	_, err := t.client.CoreV1().PersistentVolumes().Update(context.TODO(), t.persistentVolume, metav1.UpdateOptions{})
//	framework.ExpectNoError(err)
//	t.persistentVolumeClaim.Spec.StorageClassName = scName
//	_, err = t.client.CoreV1().PersistentVolumeClaims(t.namespace.Name).Update(context.TODO(), t.persistentVolumeClaim, metav1.UpdateOptions{})
//	framework.ExpectNoError(err)
//}
...
func (t *TestPersistentVolumeClaim) DeleteBackingVolume(cfs fcfs.Cfs) {
	volumeID := t.persistentVolume.Spec.CSI.VolumeHandle
	By(fmt.Sprintf("deleting FCFS volume %q", volumeID))
	vol, err := fcfsdriver.NewVolOptionsFromVolID(volumeID, nil)
	if err != nil {
		Fail(fmt.Sprintf("could not new volume options from volumeID %q: %v", volumeID, err))
	}
	credentials, err := common.NewAdminCredentials(GetParametersSec())
	if err := cfs.DeleteVolume(context.TODO(), vol, credentials); err != nil {
...
func NewTestDeployment(c clientset.Interface, ns *v1.Namespace, command string, pvc *v1.PersistentVolumeClaim, volumeName, mountPath string, readOnly bool) *TestDeployment {
	generateName := "fcfs-volume-tester-"
...
func SetupConfigMap(client clientset.Interface, ns *v1.Namespace) (*TestConfigMap, error) {
	By("setting up the Config Map")
	cm := v1.ConfigMap{}
	conmap := []common.ClusterInfo{{
		ClusterID: driver.TestOptions.ClusterID,
		ConfigURL: driver.TestOptions.ConfigURL,
	}}
	...
	cm.Name = buildConfigMapName(ns)
	configMap, err := client.CoreV1().ConfigMaps(ns.Name).Get(context.TODO(), cm.Name, metav1.GetOptions{})

	if err == nil {
		configMap, err = client.CoreV1().ConfigMaps(ns.Name).Update(context.TODO(), &cm, metav1.UpdateOptions{})
		if err != nil {
			return nil, err
		}
	}
	if apierrs.IsNotFound(err) {
		configMap, err = client.CoreV1().ConfigMaps(ns.Name).Create(context.TODO(), &cm, metav1.CreateOptions{})
	}

	return NewTestConfigMap(client, ns, configMap), nil
}

func buildConfigMapName(namespace *v1.Namespace) string {
	return fmt.Sprintf("fcfs-cm-%s", namespace.Name)
}

func (t *TestConfigMap) Cleanup() {
	e2elog.Logf("deleting config map %q/%q", t.ns.Name, t.configMap.Name)
	err := t.client.CoreV1().ConfigMaps(t.ns.Name).Delete(context.TODO(), t.configMap.Name, metav1.DeleteOptions{})
	framework.ExpectNoError(err)
}

func GetParametersSec() map[string]string {
	return map[string]string{
		"adminName":      driver.TestOptions.AdminName,
		"adminSecretKey": driver.TestOptions.AdminSecretKey,
		"userName":       driver.TestOptions.UserName,
		"userSecretKey":  driver.TestOptions.UserSecretKey,
	}
}

func SetupFcfsSecret(c clientset.Interface, namespace *v1.Namespace) (*TestSecret, error) {
	By("setting up the Secret")
	sc := v1.Secret{}
	sc.StringData = GetParametersSec()
	sc.Name = driver.TestOptions.SecretName
	sc.Namespace = namespace.Name
	sec, err := c.CoreV1().Secrets(namespace.Name).Create(context.TODO(), &sc, metav1.CreateOptions{})
	if err != nil {
		return nil, err
	}
	return NewTestSecret(c, namespace, sec), nil
}

func (t *TestSecret) Cleanup() {
	e2elog.Logf("deleting se map %q/%q", t.ns.Name, t.secret.Name)
	err := t.client.CoreV1().Secrets(t.ns.Name).Delete(context.TODO(), t.secret.Name, metav1.DeleteOptions{})
	if err != nil {
		if apierrs.IsNotFound(err) {
			return
		}
...
func SetupBaseEnv(client clientset.Interface, ns *v1.Namespace) ([]func(), error) {
	cleanupFuncs := make([]func(), 0)
	configMap, err := SetupConfigMap(client, ns)
	if err != nil {
		return nil, fmt.Errorf("could not new Config Map: %v", err)
	}
	cleanupFuncs = append(cleanupFuncs, configMap.Cleanup)
	sec, err := SetupFcfsSecret(client, ns)
	if err != nil {
		return nil, fmt.Errorf("could not new Secret: %v", err)
	}
	cleanupFuncs = append(cleanupFuncs, sec.Cleanup)
